...
public class ModWeatheringCopperSlabBlock extends SlabBlock implements ModWeatheringCopper {
    ...
    public ModWeatheringCopperSlabBlock(WeatherState weather, Properties properties, boolean canAxe) {
        super(properties);
    ...
    public void useOnBlock(BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (!worldIn.isClientSide()) {
            ...
            if (handIn.equals(InteractionHand.MAIN_HAND) && heldStack.getItem() instanceof HoneycombItem) {
                worldIn.setBlock(pos, waxedVersion.defaultBlockState().setValue(TYPE, state.getValue(TYPE)).setValue(WATERLOGGED, state.getValue(WATERLOGGED)), 1);
